...
def pick_interval(label: str, default_val: int, default_unit: str = "Minutes"):
    unit  = st.sidebar.selectbox(f"{label} unit", ("Seconds", "Minutes",
                                                   "Hours", "Days"),
                                 key=f"{label}_unit", index=
                                 {"Seconds":0,"Minutes":1,"Hours":2,"Days":3}[default_unit])
...
def shelf_cycle() -> int:
    ...
    for itemid, row in below.iterrows():
        # reuse refill_item logic inline (simplified)
        thresh = row.shelfthreshold
        avg    = row.shelfaverage
        current = row.totalquantity
        need = max(avg - current, thresh - current)
        need = SHELF.resolve_shortages(itemid=itemid,
                                       qty_need=need,
                                       user="AUTO‑UNIFIED")
        if need <= 0:
            continue
...
if RUN:
    ...
    with col2:
        ...
        st.metric("Inv rows last",
                  st.session_state.last_inv_rows)
        st.metric("Shelf qty moved",
                  st.session_state.last_sh_rows)
