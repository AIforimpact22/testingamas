...
─────────────────────────────
• Polls the warehouse inventory every 5 seconds.
• Whenever an SKU’s total stock < `inventorythreshold`
  it uses InventoryRefillHandler to create a synthetic PO and
...
def stock_snapshot() -> pd.DataFrame:
    """Return current inventory + item‑level thresholds/averages."""
    return irh._stock_levels()            # private helper is fine here


def perform_refill(df_need: pd.DataFrame, supplier_id: int) -> pd.DataFrame:
    """Restock every row in *df_need* and return an actions summary."""
    actions = []
    for _, row in df_need.iterrows():
        need = int(row.inventoryaverage) - int(row.totalqty)
        poid = irh.restock_item(
            int(row.itemid), supplier_id, need
        )
        actions.append({
...
# choose supplier used for auto POs
...
if not need_df.empty:
    ...
    actions_df = perform_refill(need_df, supplier_id)
