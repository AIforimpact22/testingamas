...
class ShelfHandler(DatabaseManager):
    ...
    def resolve_shortages(
        ...
        Returns the quantity still **uncovered** (≥ 0).

        Fully fulfilled rows are **deleted** (we never set shortage_qty = 0).
        """
        ...
        for r in rows.itertuples():
            ...
            if take == r.shortage_qty:
                # fully satisfied → delete
                self.execute_command(
                    "DELETE FROM shelf_shortage WHERE shortageid = %s;",
                    (r.shortageid,),
                )
            else:
                # partially satisfied → shrink row
                self.execute_command(
                    """
    ...
    def restock_item(self, itemid: int, *, user: str = "AUTOSIM") -> None:
        ...
        If back‑store inventory cannot cover the gap we simply exit;
        we no longer create a new shelf_shortage row because that table
        requires a non‑NULL saleid.
        """
        ...
        for lyr in layers.itertuples():
            ...
            if need <= 0:
                break
        # Any residual `need` here means both shelf and inventory are empty.
        # No further action (and no violation of NOT NULL saleid).
        return
